(function (exports) {
  ...
  var DEFAULT_PARSER, DEFAULT_SERIALIZER;
  ...
  Node.prototype.appendChild = function (node) {
    ...
    if (this instanceof Element || this instanceof Document) {
      var ancestor = traverseUpFromNode(this, function (node) {
        return node.ownerDocument;
      });

      if (ancestor && ancestor.ownerDocument) {
        traverseDownFromNode(ancestor, function (node) {
          node.ownerDocument = this;
        }, ancestor.ownerDocument);

        ancestor.ownerDocument.proxy({
          action: 'appendChild',
          workerdomid: this.workerdomid,
          value: DEFAULT_SERIALIZER.serialize(node)
        });
      }
    }
  };
  ...
  function Element(tagName) {
    ...
    this.attributes = [];
    this.workerdomid = this.getAttribute('data-workerdomid');
    this.tagName = tagName;
    this.style = {};

    if (!this.workerdomid) {
      this.workerdomid = Date.now() + '-' + parseInt(Math.random() * 500);
      this.setAttribute('data-workerdomid', this.workerdomid);
    }

    Object.defineProperty(this, 'id', {
      set: function (id) {
        this.setAttribute('id', id);
        this.postChange('id', id);
      },
      get: function () {
    ...
    Object.defineProperty(this, 'className', {
      set: function (className) {
        ...
        this.postChange('className', className);
      },
      get: function () {
    ...
    Object.defineProperty(this, 'innerHTML', {
      set: function (a) {
        ...
        innerFragment = DEFAULT_PARSER.parse(a || '');

        this.firstChild = null;
        this.lastChild = null;
        this.appendChild(innerFragment);
        this.postChange('innerHTML', DEFAULT_SERIALIZER.serialize(innerFragment));
      },
      get: function () {
        return innerFragment ? DEFAULT_SERIALIZER.serialize(innerFragment) : '';
      }
    });
  }
  ...
  Element.prototype.postChange = function (key, value) {
    if (this.ownerDocument) {
      this.ownerDocument.proxy({action: key, workerdomid: this.workerdomid, value: value});
    }
  };
  ...
  function Document(proxy) {
    this.proxy = typeof proxy === 'function' ? proxy : function () {
    };
    this.location = {};
  ...
  function traverseUpFromNode(node, perform, context) {
    if (!node) return;
    if (perform.call(context, node)) return node;

    traverseUpFromNode(node.parentNode, perform, context);
  }

  function traverseDownFromNode(node, perform, context) {
    if (!node) return;
    if (perform.call(context || node, node)) return node;

    var child = node.firstChild;

    while (child) {
      traverseDownFromNode(child, perform, context);
      child = child.nextSibling;
    }
  }

  function recursiveElementSearch(attr, value, node, nodes, max) {
    if (!node || node.nodeType !== 1) return;
    if ((node[attr] || '').indexOf(value) > -1) {
      nodes.push(node);
      if (nodes.length === max) {
        return nodes;
      }
    ...
    while (child) {
      recursiveElementSearch(attr, value, child, nodes, max);
      child = child.nextSibling;
    }
  ...
  Document.prototype.getElementsByClassName = function (className) {
    var nodes = [];
    traverseDownFromNode(this.documentElement, function (node) {
      if (node.nodeType == 1 && node.getAttribute('className').indexOf(className) > -1) {
        nodes.push(node);
      }
    });
  ...
  Document.prototype.getElementsByTagName = function (tagName) {
    var nodes = [];
    traverseDownFromNode(this.documentElement, function (node) {
      if (node.nodeType == 1 && node.nodeName.indexOf(tagName.toUpperCase()) > -1) {
        nodes.push(node);
      }
    });
  ...
  HTMLParser.prototype.pushElement = function (token) {
    ...
    for (var i = 0; i < token.attributes.length; i++) {
      ...
      if (attr[0] === 'data-workerdomid') {
        el.workerdomid = attr[1];
  ...
  HTMLSerializer.prototype.attr = function (attr) {
    ...
    if (attr.value) {
      return ' ' + name + '="' + this.escapeAttrValue(attr.value) + '"';
    }
    return ' ' + name;
  };
  ...
  DEFAULT_SERIALIZER = new HTMLSerializer(_voidMap);
  DEFAULT_PARSER = new HTMLParser(window.tokenize, new Document(), _voidMap);
